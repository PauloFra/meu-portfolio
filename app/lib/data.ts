export const projects = [
  {
    title: "3D Portfolio",
    description: "Um portfólio interativo em 3D com navegação fluida.",
    tech: ["Next.js", "Three.js", "Tailwind"],
    image: "/images/3d-portfolio.jpg",
    slug: "3d-portfolio",
    link: "https://github.com/seu-usuario/3d-portfolio",
  },
  {
    title: "Dashboard SaaS",
    description: "Painel de controle com dados em tempo real.",
    tech: ["Next.js", "TypeScript", "Supabase"],
    image: "/images/dashboard.jpg",
    slug: "dashboard-saas",
    link: "https://github.com/seu-usuario/dashboard-saas",
  },
];

export const skills = [
  {
    name: "React",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/main/icons/react.png",
  },
  {
    name: "Next.js",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/next_js.png",
  },
  {
    name: "TypeScript",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/typescript.png",
  },
  {
    name: "Node.js",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/node_js.png",
  },
  {
    name: "Tailwind CSS",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/tailwind_css.png",
  },
  {
    name: "GraphQL",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/graphql.png",
  },
  {
    name: "AWS",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/aws.png",
  },
  {
    name: "Docker",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/docker.png",
  },
  {
    name: "Git",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/git.png",
  },
  {
    name: "Figma",
    icon: "https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/figma.png",
  },
];
export const experiences = [
  {
    title: "Software Engineer PL",
    company: "DBC Company / Usaflex",
    period: "Março 2022 - Presente",
    description:
      "Atuo  no desenvolvimento da nova plataforma de e-commerce da Usaflex, utilizando VTEX IO, ReactJS e NodeJS. Implementação de novas funcionalidades e integrações dentro do ecossistema OMNI Channel. \n\n" +
      "Atuação em System Design, Performance Optimization, Requirements Analysis, Prototyping, Risk Management e Documentation. \n\n" +
      "Apoio aos desenvolvedores através de pair programming e code reviews. \n\n" +
      "Liderança de squad focada no desenvolvimento do checkout, incluindo integrações com terceiros e otimizações de performance. \n\n" +
      "Experiência com DevOps: AWS, Azure, Docker e Bash Scripting. \n\n" +
      "Desenvolvimento de microservices escaláveis utilizando Node.js e GraphQL.",
    technologies: [
      "VTEX IO",
      "React.js",
      "Node.js",
      "GraphQL",
      "AWS",
      "Azure",
      "Docker",
      "Bash Scripting",
    ],
  },
  {
    title: " Desenvolvimento Web",
    company: "DBC Company",
    period: "Fevereiro 2022 - Junho 2022",
    description:
      "Participação no desenvolvimento de aplicações web utilizando React, TypeScript e Redux. Implementação de funcionalidades interativas e otimização da experiência do usuário. \n\n" +
      "Desenvolvimento de um sistema de gerenciamento de candidatos e vagas, incluindo módulo de login, fluxo de matrícula e painel administrativo para gestão de status dos candidatos.",
    technologies: ["React.js", "TypeScript", "Redux", "Styled Components"],
  },
];
